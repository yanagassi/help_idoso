...
import { Icon ,Permissions, FileSystem } from 'expo';
...
export default class CentralDownloads extends React.Component {
    ...
    async criarDownload(fileName = '', downloadSource = '', downloadTitle = '', data = {}){
        ...
        try{
            ...
            downloadObject = new DownloadObject({
                ...
                downloadStatusChangeCallback: (newStatus)=>{}, //quando status do download mudar
    ...
    _downloadLicao(licao = {}, CURSO_ID = null){     //Iniciei a Função de Donwload.
        if(licao === {}) return; 
        if(typeof CURSO_ID !== "string") return; 
        this._showMessage("Seu download começará em breve.");
        fetch(`https://api.voitto.com.br/App_ios/download_video?js_id=${licao.lic_js_id}&curso_id=${CURSO_ID}`)
        .then(res=>res.json()).then(res=>{
            this.criarDownload(`${CURSO_ID}${licao.lic_js_id}.mp4`, res.link, licao.titulo, licao);
        }).catch(e=>{
                    ...
                    <Modal
                        ...
                        <View style={{flex:1, backgroundColor:Colors.primary, height: Layout.window.height}}>
                            <View style={{marginTop: 40, height: 50, flexDirection: 'row', alignItems:'center'}}>
                                ...
                                <Text style={styles.title}>Central de Downloads</Text>
                            </View>
